...
def get_swimmers_data(name, age, distance, stroke):
    ...
    with UseDatabase(DB_CONFIG) as cursor:
        _SQL = f"""SELECT t.time
        FROM times t
        INNER JOIN events e ON t.event_id = e.id
        INNER JOIN swimmers s ON t.swimmer_id = s.id
        WHERE s.name = '{name}' AND s.age = {age} AND e.distance = {distance} AND e.stroke = '{stroke}'"""
